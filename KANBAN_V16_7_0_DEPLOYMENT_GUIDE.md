...
## 📝 OnChange Formula (Canvas App)

The Canvas App uses the following `OnChange` formula to handle all Kanban operations:

```powerapps
Switch(Self.outputEvent,

    /* EDIT_OPEN: Lock task data for fallback */
    "EDIT_OPEN",
    With({ Data: ParseJSON(Self.outputData) },
        Set(varK_CurrentTask, 
            LookUp('Developer Tasks', lto_developertasksid = GUID(Text(Data.itemId)))
        )
    ),

    /* UPDATE: Main save logic */
    "UPDATE",
    With({ Data: ParseJSON(Self.outputData) },
        Patch(
            'Developer Tasks',
            LookUp('Developer Tasks', lto_developertasksid = GUID(Text(Data.itemId))),
            {
                lto_taskname: Text(Data.title),
                lto_projectfeature: Coalesce(Text(Data.subtitle), Text(Data.model)),
                lto_storypoints: Value(Text(Data.storyPoints)),
                
                lto_status: With({ Raw: Text(Data.statusId) },
                    Switch(Raw,
                        "Backlog", 'Status (Developer Tasks)'.Backlog, "100000000", 'Status (Developer Tasks)'.Backlog,
                        "In Progress", 'Status (Developer Tasks)'.'In Progress', "100000001", 'Status (Developer Tasks)'.'In Progress',
                        "Code Review", 'Status (Developer Tasks)'.'Code Review', "100000002", 'Status (Developer Tasks)'.'Code Review',
                        "Done", 'Status (Developer Tasks)'.Done, "100000003", 'Status (Developer Tasks)'.Done,
                        'Status (Developer Tasks)'.Backlog
                    )
                ),

                'Assigned To': With({ UID: Text(Data.assignedToId) },
                    If(
                        IsBlank(UID) Or UID = "null" Or UID = "", 
                        varK_CurrentTask.'Assigned To',
                        LookUp(Users, systemuserid = GUID(UID))
                    )
                ),

                lto_type: ForAll(
                    Split(Text(Data.types), ","),
                    With({ CID: Trim(ThisRecord.Value) },
                        LookUp(
                            Choices('Developer Tasks'.lto_type), 
                            Value = Switch(CID,
                                "UX", 100000000,
                                "UI", 100000001,
                                "Development", 100000002,
                                "Testing", 100000003,
                                "Architecture", 100000004,
                                "Documentation", 100000005,
                                "Project Management", 100000006,
                                Value(CID)
                            )
                        )
                    )
        ...
        Refresh('Developer Tasks')
    ),

    /* CREATE */
    "CREATE",
    With({ Data: ParseJSON(Self.outputData) },
        Patch('Developer Tasks', Defaults('Developer Tasks'), {
            lto_taskname: Text(Data.title),
            lto_projectfeature: Coalesce(Text(Data.subtitle), Text(Data.model)),
            lto_storypoints: Value(Text(Data.storyPoints)),
            lto_status: Switch(Text(Data.statusId), "100000000", 'Status (Developer Tasks)'.Backlog, "100000001", 'Status (Developer Tasks)'.'In Progress', "100000002", 'Status (Developer Tasks)'.'Code Review', "100000003", 'Status (Developer Tasks)'.Done, 'Status (Developer Tasks)'.Backlog),
            'Assigned To': If(IsBlank(Text(Data.assignedToId)), Blank(), LookUp(Users, systemuserid = GUID(Text(Data.assignedToId)))),
            lto_type: ForAll(Split(Text(Data.types), ","), With({ CID: Trim(ThisRecord.Value) }, LookUp(Choices('Developer Tasks'.lto_type), Value = Switch(CID, "UX", 100000000, "UI", 100000001, "Development", 100000002, "Testing", 100000003, "Architecture", 100000004, "Documentation", 100000005, "Project Management", 100000006, Value(CID)))))
        });
        Refresh('Developer Tasks')
    ),
    ...
    "UPDATE_STATUS",
    With({ Data: ParseJSON(Self.outputData) },
        Patch('Developer Tasks', LookUp('Developer Tasks', lto_developertasksid = GUID(Text(Data.itemId))), { lto_status: Switch(Text(Data.newColumnId), "100000000", 'Status (Developer Tasks)'.Backlog, "100000001", 'Status (Developer Tasks)'.'In Progress', "100000002", 'Status (Developer Tasks)'.'Code Review', "100000003", 'Status (Developer Tasks)'.Done, 'Status (Developer Tasks)'.Backlog) });
        Refresh('Developer Tasks')
    ),
    ...
    "DELETE",
    With({ Data: ParseJSON(Self.outputData) },
        Remove('Developer Tasks', LookUp('Developer Tasks', lto_developertasksid = GUID(Text(Data.itemId))));
        Refresh('Developer Tasks')
    )
)
```
